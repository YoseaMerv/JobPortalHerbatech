                ...
                <div class="tab-pane fade show active" id="identitas" role="tabpanel">
                    <div class="card border-0 shadow-sm rounded-4">
                        <div class="card-body p-4 p-md-5">
                            ...
                            <p class="text-muted mb-4 small">Lengkapi data diri Anda secara akurat untuk mempermudah proses seleksi HR.</p>
                            ...
                            <form action="{{ route('seeker.profile.update') }}" method="POST" enctype="multipart/form-data">
                                ...
                                <div class="row g-4">
                                    ...
                                    <div class="col-md-12 mb-3 text-center">
                                        ...
                                        <div class="mt-2 text-center">
                                            <label class="form-label-custom d-block">Foto Profil Profesional</label>
                                        </div>
                                    </div>
                                    ...
                                    <div class="col-md-6">
                                        <label class="form-label-custom">Nomor Telepon (WhatsApp) <span class="text-danger">*</span></label>
                                        <input type="text" name="phone" class="form-control input-style" value="{{ old('phone', $profile?->phone) }}" placeholder="Contoh: 08123456789" required {{ $isLocked ? 'readonly' : '' }}>
                                    </div>
                                    <div class="col-md-6">
                                        <label class="form-label-custom">Kota Domisili <span class="text-danger">*</span></label>
                                        <input type="text" name="home_location_details" class="form-control input-style" value="{{ old('home_location_details', $profile?->home_location_details) }}" placeholder="Contoh: Jakarta Selatan" required {{ $isLocked ? 'readonly' : '' }}>
                                    </div>

                                    {{-- Kolom Baru yang Ditambahkan --}}
                                    <div class="col-md-6">
                                        <label class="form-label-custom">Tanggal Lahir</label>
                                        <input type="date" name="date_of_birth" class="form-control input-style" value="{{ old('date_of_birth', $profile?->date_of_birth ? \Carbon\Carbon::parse($profile->date_of_birth)->format('Y-m-d') : '') }}" {{ $isLocked ? 'readonly' : '' }}>
                                    </div>
                                    <div class="col-md-6">
                                        <label class="form-label-custom">Jenis Kelamin</label>
                                        <select name="gender" class="form-select input-style" {{ $isLocked ? 'disabled' : '' }}>
                                            <option value="" disabled {{ empty($profile?->gender) ? 'selected' : '' }}>Pilih Jenis Kelamin</option>
                                    ...
                                    <div class="col-md-6">
                                        <label class="form-label-custom">URL LinkedIn / Portofolio</label>
                                        <input type="url" name="linkedin_url" class="form-control input-style" value="{{ old('linkedin_url', $profile?->linkedin_url) }}" placeholder="https://linkedin.com/in/username" {{ $isLocked ? 'readonly' : '' }}>
                                    </div>
                                    <div class="col-md-6">
                                        <label class="form-label-custom">Gaji yang Diharapkan (IDR)</label>
                                        <div class="input-group">
                                            <span class="input-group-text border-0 bg-light" style="border-radius: 12px 0 0 12px;">Rp</span>
                                            <input type="number" name="expected_salary" class="form-control input-style" style="border-radius: 0 12px 12px 0;" value="{{ old('expected_salary', $profile?->expected_salary) }}" placeholder="Contoh: 5000000" {{ $isLocked ? 'readonly' : '' }}>
                                        </div>
                                    </div>
                                ...
                                <div class="text-end mt-5">
                                    <button type="submit" class="btn btn-primary px-5 py-2 fw-bold rounded-pill shadow-sm transition-all hover-lift">Simpan Identitas</button>
                                </div>
                                @endif
                            </form>
                        </div>
                        ...
                        <form action="{{ route('seeker.profile.update') }}" method="POST">
                            ...
                            <div class="mb-5">
                                ...
                                <textarea name="about" class="form-control input-style p-3" rows="6"
                                    placeholder="Ceritakan keahlian, pengalaman, dan minat profesional Anda..." {{ $isLocked ? 'readonly' : '' }}>{{ old('about', $profile?->summary) }}</textarea>
                            </div>
                            ...
                            <div class="mb-4">
                                ...
                                <div class="row g-3">
                                    @php 
                                        ...
                                        $savedLangs = old('languages', is_array($profile?->languages) ? $profile->languages : []);
                                    @endphp
                                    ...
                                    <div class="col-6 col-md-3">
                                        <div class="language-card border rounded-3 p-3 text-center position-relative transition-all {{ $isLocked ? 'opacity-75' : '' }}">
                                            <input class="form-check-input stretched-link d-none" type="checkbox" name="languages[]" value="{{ $lang }}" id="lang{{ $lang }}"
                                                {{ in_array($lang, $savedLangs) ? 'checked' : '' }} {{ $isLocked ? 'disabled' : '' }}>
                            ...
                            @if(!$isLocked)
                            <div class="text-end mt-5">
                                <button type="submit" class="btn btn-primary px-5 py-2 fw-bold rounded-pill shadow-sm transition-all hover-lift">Simpan Bio</button>
                            </div>
                            @endif
                        </form>
                    </div>
...
<style>
    ...
    .list-group-item { transition: all 0.2s; border: none; cursor: pointer; }
    .list-group-item.active {
        background-color: #0d6efd !important; color: white !important;
        box-shadow: 0 4px 12px rgba(13, 110, 253, 0.2);
    }
    .list-group-item.active .icon-box { background-color: rgba(255, 255, 255, 0.2); color: white; }
    .input-style {
        background-color: #fcfcfc; border: 1.5px solid #f1f3f5;
        border-radius: 12px; padding: 12px 16px; font-size: 0.95rem; transition: all 0.2s;
    }
    .input-style:focus {
        background-color: #fff; border-color: #0d6efd;
        box-shadow: 0 0 0 4px rgba(13, 110, 253, 0.05); outline: none;
    }
    .form-label-custom {
        font-size: 0.65rem; font-weight: 800; color: #adb5bd;
        letter-spacing: 0.08rem; margin-bottom: 8px; text-transform: uppercase;
    }
    .border-dashed { border-style: dashed !important; border-width: 2px !important; border-color: #dee2e6 !important; }
    .transition-all { transition: all 0.3s ease; }
    .hover-lift:hover { transform: translateY(-2px); }
    .hover-shadow:hover { box-shadow: 0 8px 20px rgba(0, 0, 0, 0.05) !important; }
    .hover-opacity-100:hover { opacity: 1 !important; }
    .language-card label {
        padding: 14px; border: 2px solid #f1f3f5; border-radius: 14px;
        cursor: pointer; display: block; font-size: 0.85rem; background: #fff;
    }
    ...
    .timeline-item::before {
        ...
        height: 100%; width: 2px; background-color: #f1f3f5;
    }
    .timeline-dot {
        position: absolute; left: -4px; top: 5px; width: 10px; height: 10px;
        background-color: #0d6efd; border-radius: 50%; border: 2px solid white;
