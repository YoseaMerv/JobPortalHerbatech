...
<div class="container py-4">
    {{-- LOGIKA TAMBAHAN: Alert Status Accepted --}}
    @if(auth()->user()->applications()->where('status', 'accepted')->exists())
    <div class="alert alert-info border-0 shadow-sm d-flex align-items-center mb-4 rounded-4 p-3">
        <i class="fas fa-check-circle fs-4 me-3 text-info"></i>
        <div>
            <h6 class="fw-bold mb-0">Status: Sudah Diterima</h6>
            <small>Anda sudah diterima di sebuah posisi. Anda tidak dapat melamar lowongan lain saat ini.</small>
        </div>
    </div>
    @endif
    <div class="container py-4">
                ...
                <div class="card border-0 shadow-sm rounded-4 h-100 transition-all hover-shadow">
                    <div class="card-body p-4">
                        ...
                        <div class="d-flex justify-content-between align-items-center mt-auto pt-3 border-top">
                            ...
                            {{-- LOGIKA TAMBAHAN: Cek status untuk tombol --}}
                            @if(auth()->user()->applications()->where('status', 'accepted')->exists())
                            <button class="btn btn-light btn-sm rounded-pill px-3 fw-bold text-muted" disabled>
                                <i class="fas fa-lock me-1 small"></i> Terkunci
                            </button>
                            @else
